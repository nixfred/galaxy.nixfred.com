...
1. **Interface word list.** `02_ART_DIRECTION.md` proposed `SIGNALS` as a top-level navigation label alongside `MAP`, `TIMELINE`, `TOURS`, and `SEARCH`. This collides with `Signal`, which is a sector name (accepted default, `01_INTERVIEW.md` Q26 to Q27, kept as `SIGNAL`), and the functional requirements never define a `SIGNALS` mode. The finalized product requirements instead define `ATLAS` as a first-class mode (FR048 to FR056, AC021 to AC023) and `SURPRISE ME` as a named random-discovery action (FR059, accepted default Q41), neither of which appear in the `02_ART_DIRECTION.md` list. Resolution: the Interface vocabulary section above is built from the FR-confirmed feature set. `SIGNAL` is used only as a sector name and never as a navigation command.
...
4. **Easter egg scope.** `03_ART_DIRECTION_DRAFT.md` recommended four additional command easter eggs beyond Sky Walrus, tracing back to `06_DISCOVERY_INTERVIEW.md` Q93 to Q100. Resolution: CUT by F21. Only Sky Walrus ships in v1; the typed `whoami`, `larry`, and `home` commands and the Konami-code rendering are removed from scope.
5. **Typography specifics.** `02_ART_DIRECTION.md` says to reuse the existing nixfred.com type families conditionally. `03_ART_DIRECTION_DRAFT.md` gives a clean three-tier hierarchy without naming families. `06_DISCOVERY_INTERVIEW.md` Q101 to Q103 ask Fred to confirm the exact palette and type family names, unanswered as of this document. Resolution: the three-tier hierarchy is locked; the specific family names are PENDING-FRED and must be pulled from nixfred.com's actual production type stack at implementation time rather than guessed.
